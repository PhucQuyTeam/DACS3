                        ...
                        if (response.isSuccessful) {
                            ...
                            if (jsonObject.has("return_code") && jsonObject.getInt("return_code") == 1) {
                                ...
                                prefs.edit()
                                    ...
                                    .apply()
                                if (orderUrl.contains("fake")) {
                                    // giả lập delay rồi quay lại luôn
                                    lifecycleScope.launch {
...
//                                val browserIntent = Intent(Intent.ACTION_VIEW, Uri.parse(orderUrl))
//                                startActivity(browserIntent)
////                                Toast.makeText(this@CheckoutActivity, "Tạo đơn thành công! Hãy copy link ở Backend dán vào web.", Toast.LENGTH_LONG).show()
                            } else {
        ...
        private fun checkPaymentStatus(appTransId: String) {
            val isFake = appTransId.startsWith("demo_")
            val msg = if (isFake) "Đang xử lý thanh toán " else "Đang kiểm tra giao dịch..."
